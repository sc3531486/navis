import { appState, openRightWorkspacePanel, type RightWorkspacePanel } from './app';
...
function openRightWorkspaceIds(): Set<string> {
  return new Set(
    appState.rightWorkspaceColumns.flatMap((column) => column.panels.map((panel) => panel.id)),
  );
}
