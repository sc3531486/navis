...
### 5.7 浮层/弹窗插槽 (`overlay`) 与鼠标交互规范 (Pointer Events Hygiene)

宿主在 `WhiteboardShell` 中提供的顶级 `overlay` 动态插槽（用于承载全局弹窗、命令面板、右键上下文菜单等），为了保证在没有弹窗激活时不遮挡底层主工作区的正常鼠标交互，其父级容器（`.navis-overlay-layer`）默认声明了 `pointer-events: none;`。

**扩展开发核心守则**：
- 任何挂载到 `overlay` 插槽的对话框、模态弹窗（如 `SettingsModal`）、命令面板（如 `CommandPalette`）或浮动气泡，其最外层全屏遮罩（Mask / Backdrop）或弹窗主体容器，**必须显式声明 `pointer-events: auto;`**。
- **反模式警示**：若遗漏 `pointer-events: auto;`，整个弹窗将继承父层的 `pointer-events: none;`，导致真实用户的物理鼠标点击事件完全无法被弹窗内的任何按钮、菜单、Tab 或输入框捕获，产生“UI 渲染正常但所有按钮点击均无任何反应”的严重缺陷。

```tsx
// ✅ 正确范例：全屏遮罩与弹窗卡片均显式设置 pointer-events: auto
<Show when={open()}>
  <div
    onClick={() => setOpen(false)}
    style="position: fixed; inset: 0; background: rgba(0,0,0,0.45); z-index: 9999; display: flex; align-items: center; justify-content: center; pointer-events: auto;"
  >
    <div
      onClick={(e) => e.stopPropagation()}
      style="width: 920px; background: #ffffff; border-radius: 14px; pointer-events: auto;"
    >
      {/* 弹窗内容与全部按钮 */}
    </div>
  </div>
...
### 5.8 滚动容器与鼠标滚轮事件隔离规范 (Mouse Wheel & Overscroll Hygiene)

在桌面端应用中，弹窗、列表与终端面板等可滚动区域需要接受**鼠标滚轮（Mouse Wheel / Trackpad Scroll）**的物理滚动。

**扩展开发核心守则**：
1. **滚轮事件捕获前提**：滚轮事件属于鼠标物理交互事件，同样依赖 `pointer-events: auto;`。如果浮层弹窗缺失该属性，鼠标滚轮将被父容器拦截或透传到底层主窗口。
2. **避免滚动穿透与滚动链（Scroll Chaining）**：
   - 弹窗内的可滚动容器（如模型列表、设置项、会话历史树）必须显式声明 `overscroll-behavior: contain;`；
   - 这样当用户使用鼠标滚轮快速滚动到底部或顶部时，浏览器不会将滚轮动量传递给弹窗背后的主白板工作区，避免造成底层界面意外位移。
3. **滚动条优雅降级与自定义**：
   - 滚动容器应使用 `overflow-y: auto;` 并配合极简细滚动条样式，保证长内容下的视觉整洁。

```css
/* ✅ 规范的可滚动扩展容器样式 */
.my-extension-scroll-container {
  flex: 1;
  overflow-y: auto;
  overscroll-behavior: contain; /* 锁住滚轮事件，严禁滚动穿透到宿主底座 */
}
```
