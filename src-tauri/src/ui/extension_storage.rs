...
impl ExtensionEphemeralStorage {
    fn get(&self, key: &str) -> Result<Option<Value>, String> {
        let values = self.values.lock().map_err(|_| "extension ephemeral storage mutex poisoned".to_string())?;
        Ok(values.get(key).cloned())
    }

    fn set(&self, key: String, value: Value) -> Result<(), String> {
        let mut values = self.values.lock().map_err(|_| "extension ephemeral storage mutex poisoned".to_string())?;
...
fn ensure_storage_allowed(
    ...
    let Some(declared) = &state.manifest.contributes.storage else {
        return Err(format!("Extension '{extension_id}' did not declare storage scopes"));
    };
...
fn require_allowed(result: &CheckResult, operation: &str) -> Result<(), String> {
    if result.allowed && !result.require_confirm {
        ...
        Err(format!("Extension storage '{operation}' denied by sandbox: {reason}"))
    }
}
...
pub fn clear_extension_ephemeral(ephemeral: &ExtensionEphemeralStorage, extension_id: &str) -> Result<(), String> {
...
/// 文件目录布局处理。
fn storage_key(extension_id: &str, scope: &StorageScope, worktree: Option<&str>, key: &str) -> Result<String, String> {
    validate_key(key)?;
    match scope {
        StorageScope::Global => Ok(format!("extension:{extension_id}:global:{key}")),
        StorageScope::Worktree => Ok(format!("extension:{extension_id}:worktree:{}:{key}", worktree_hash(worktree)?)),
        StorageScope::Ephemeral => Ok(format!("extension:{extension_id}:ephemeral:{key}")),
    }
}

/// Sandbox 门禁目标命名空间前缀（clear 用）。
fn storage_prefix(extension_id: &str, scope: &StorageScope, worktree: Option<&str>) -> Result<String, String> {
    match scope {
        StorageScope::Global => Ok(format!("extension:{extension_id}:global:")),
        StorageScope::Worktree => Ok(format!("extension:{extension_id}:worktree:{}:", worktree_hash(worktree)?)),
...
fn facade_for_state(state: &ExtensionState) -> Result<ExtensionStorage, String> {
    let base_dir = state
        ...
        .parent()
        .map(PathBuf::from)
        .ok_or_else(|| format!("Extension '{}' install path has no parent directory", state.id))?;
...
pub fn ui_extension_storage_get(
    ...
    let state = ensure_storage_allowed(extension_store.inner().as_ref(), &request.extension_id, &scope)?;
    let key = storage_key(&request.extension_id, &scope, request.worktree.as_deref(), &request.key)?;
    // 只读访问：轻量 Sandbox FileRead 门禁 + 审计（manifest 声明校验是第一道防线）
    sandbox_storage_check(mcp.sandbox().as_ref(), &request.extension_id, OperationType::FileRead, key.clone())?;
    let value = if scope == StorageScope::Ephemeral {
        ephemeral.get(&key)?
    } else {
        let facade = facade_for_state(&state)?;
        facade.get(&request.extension_id, &scope, request.worktree.as_deref(), &request.key)?
    };
...
pub fn ui_extension_storage_set(
    ...
    sandbox_storage_check(mcp.sandbox().as_ref(), &request.extension_id, OperationType::FileWrite, key.clone())?;
    let value = request.value.ok_or_else(|| "storage.set requires value".to_string())?;
...
pub fn ui_extension_storage_delete(
    ...
    sandbox_storage_check(mcp.sandbox().as_ref(), &request.extension_id, OperationType::FileDelete, key.clone())?;
    if scope == StorageScope::Ephemeral {
        ephemeral.delete(&key)
    } else {
        let facade = facade_for_state(&state)?;
        facade.delete(&request.extension_id, &scope, request.worktree.as_deref(), &request.key)
    }
}
...
pub fn ui_extension_storage_clear(
    ...
    sandbox_storage_check(mcp.sandbox().as_ref(), &request.extension_id, OperationType::FileDelete, prefix.clone())?;
    if scope == StorageScope::Ephemeral {
        ephemeral.clear_prefix(&prefix)
    } else {
        let facade = facade_for_state(&state)?;
...
mod tests {
    ...
    fn test_sandbox() -> Sandbox {
        ...
        Sandbox::new(Arc::new(InMemoryEventBus::new(1000, runtime.handle().clone())))
    }
    ...
    fn clear_extension_ephemeral_removes_only_matching_extension() {
        ...
        assert!(ephemeral.get("extension:alpha:ephemeral:foo").unwrap().is_none());
        assert_eq!(
    ...
    fn storage_key_namespaces_scopes() {
        ...
        let worktree = storage_key("ext.alpha", &StorageScope::Worktree, Some("/repo"), "theme").unwrap();
