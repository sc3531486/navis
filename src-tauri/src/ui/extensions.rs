...
use super::dto::{
    UiExtensionConfiguration, UiExtensionConfigurationUpdate, UiExtensionContributionCounts,
    UiExtensionDiscoveryResult, UiExtensionLocale, UiExtensionPointRegistration, UiExtensionScript,
    UiExtensionState, UiExtensionView, UiRegisteredWorkMode, UiWorkModeModelPreferences,
    UiWorkModeRegistration, UiZone,
};
...
fn csp_for_network(policy: &NetworkPolicy) -> String {
    match policy {
        NetworkPolicy::None => {
            "default-src 'none'; script-src 'unsafe-inline'; style-src 'unsafe-inline'; img-src 'none'; connect-src 'none'; font-src 'none'; frame-src 'none';"
                .to_string()
        }
        ...
        NetworkPolicy::Allowlist { hosts } => {
            ...
            format!(
                "default-src 'none'; script-src 'unsafe-inline'; style-src 'unsafe-inline'; img-src {sources}; connect-src {sources}; font-src {sources}; frame-src 'none';"
            )
        }
...
pub async fn ui_read_extension_entry_html(
    ...
    let request =
        OperationRequest::new(OperationType::FileRead, display.clone(), "host".to_string());
    ...
    Ok(inject_entry_csp(
        &html,
        &network_policy_for_state(&extension),
    ))
}
...
pub fn ui_list_zones(
    ...
    let mut zones = vec![
        UiZone {
            id: "rightWorkspace".into(),
            name: "Right workspace".into(),
            kind: "builtin".into(),
            extension_id: None,
            anchor_parent: None,
        ...
        UiZone {
            id: "bottomDrawer".into(),
            name: "Bottom drawer".into(),
            kind: "builtin".into(),
            extension_id: None,
            anchor_parent: None,
            anchor_position: None,
    ...
    for extension in extension_store.inner().as_ref().list() {
        ...
        let Some(projection) = projections.get(&extension.id) else {
            continue;
        };
            ...
            projections
                ...
                .flat_map(move |projection| {
                    ...
                    projection.scripts.into_iter().map(move |script| {
                        let resource_path = crate::ui::host_view::extension_resource_path(
                            &install_path,
                            &script.entry,
                        )
                        .ok()
    ...
    extension_store
        ...
        .flat_map(|extension| {
            extension
                ...
                .map(move |locale| {
                    let resource_path =
                        resolve_extension_manifest_entry(&extension.install_path, &locale.entry)
                            .ok()
                            .map(|path| path.display().to_string());
        ...
        .filter_map(|extension| {
            ...
            let provides = extension
                ...
                .unwrap_or_else(|| {
                    extension
                        ...
                        .capabilities
                        .as_ref()
                        .map(|caps| caps.provides.clone())
                        .unwrap_or_default()
                });

            if let Some(required) = query.provides.as_deref() {
                if !provides.iter().any(|item| item == required) {
                    return None;
                }
            }
            ...
            if let Some(capability) = query.capability.as_deref() {
                let has_capability = match capability {
                    ...
                    "network" => {
                        extension.manifest.contributes.network.is_some()
                            || extension
                                .manifest
                                .contributes
                                .capabilities
                                .as_ref()
                                .and_then(|caps| caps.network.as_ref())
                                .is_some()
                    }
...
pub fn ui_list_extension_points(
    ...
    for extension in extension_store.inner().as_ref().list() {
        ...
        for item in &projection.toolbar_items {
            points.push(UiExtensionPointRegistration {
                extension_id: extension.id.clone(),
                kind: "toolbar".into(),
                id: item.id.clone(),
                label: Some(item.label.clone()),
                command: Some(item.command.clone()),
                target: Some(format!("{:?}", item.position)),
        ...
        for item in &projection.inline_extensions {
            points.push(UiExtensionPointRegistration {
                ...
                command: None,
                target: Some(format!("{:?}", item.target)),
                group: None,
                when: item.when.clone(),
                data: serde_json::to_value(item).unwrap_or_default(),
            });
        ...
        if let Some(configuration) = &projection.configuration {
            points.push(UiExtensionPointRegistration {
                ...
                id: format!("{}:configuration", extension.id),
                label: Some(extension.manifest.name.clone()),
                command: None,
                target: Some("settingsSection".into()),
                group: None,
                when: None,
...
fn validate_configuration_value(
    ...
    if depth > 12 {
        return Err(format!(
            "Configuration value is too deeply nested at {path}"
        ));
    }
    ...
    if let Some(options) = schema.get("enum").and_then(Value::as_array) {
        if !options.iter().any(|option| option == value) {
            return Err(format!(
                "Configuration value at {path} is not in the declared enum"
            ));
        }
    }
    ...
    if let Some(expected) = schema.get("type").and_then(Value::as_str) {
        ...
        if !valid {
            return Err(format!(
                "Configuration value at {path} must have type {expected}"
            ));
        }
    }
        ...
        if let Some(required) = schema.get("required").and_then(Value::as_array) {
            for key in required.iter().filter_map(Value::as_str) {
                if !object.contains_key(key) {
                    return Err(format!(
                        "Missing required configuration property {path}.{key}"
                    ));
                }
            }
        }
