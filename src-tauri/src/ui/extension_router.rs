...
fn caller_allowed(
        ...
        .is_some_and(|caps| {
            caps.extension_calls.iter().any(|call| {
                (call.target == target_extension_id || call.target == "*")
                    && (call.actions.iter().any(|item| item == action)
                        || call.actions.iter().any(|item| item == "*"))
            })
        });
    ...
    if allowed {
        ...
        Err(format!("Caller extension '{caller_id}' is not allowed to perform '{action}' on '{target_extension_id}'"))
    }
}
...
fn target_exported(
    ...
    if target.status != ExtensionStatus::Enabled {
        return Err(format!(
            "Target extension '{target_extension_id}' is not enabled"
        ));
    }
    ...
    let exported = match action {
        "view.open" | "view.toggle" => {
            exports.is_some_and(|exports| exports.views.iter().any(|view| view == local_id))
        }

        "command.execute" | "message.send" => exports
            .is_some_and(|exports| exports.commands.iter().any(|command| command == local_id)),
    ...
    if exported {
        ...
        Err(format!(
            "Target '{target_extension_id}:{local_id}' does not export action '{action}'"
        ))
    }
}
...
mod tests {
    ...
    fn caller_allowed_honors_exact_and_wildcard() {
        ...
        register(
            ...
            caller_contributes("target", vec!["view.open"]),
        );
    ...
    fn target_exported_requires_enabled_target_and_export() {
        ...
        register(
            ...
            "target",
            ExtensionStatus::Enabled,
            target_contributes(vec!["panel"], vec!["run"]),
        );
    ...
    fn route_call_authorizes_both_sides_and_audits_success() {
        ...
        register(
            ...
            "target",
            ExtensionStatus::Enabled,
            target_contributes(vec!["panel"], vec![]),
        );
    ...
    fn route_call_rejects_unexported_target_and_audits_failure() {
        ...
        register(
            ...
            "caller",
            ExtensionStatus::Enabled,
            caller_contributes("target", vec!["command.execute"]),
        );
        ...
        register(
            ...
            "target",
            ExtensionStatus::Enabled,
            target_contributes(vec![], vec!["run"]),
        );
